...
def is_image_only_pdf(file_path: str, text_threshold: int = 20) -> bool:
    """
    Check if a PDF is image-only (like Canva PDFs or scanned documents).
    
    Args:
        file_path: Path to the PDF file
        text_threshold: Minimum number of characters to consider it text-based (default: 20)
    
    Returns:
        True if the PDF appears to be image-only, False if it has extractable text
    """
    try:
        ...
        # If total text is below threshold, it's likely image-only
        is_image_only = total_text_length < text_threshold
        
        if is_image_only:
            print(f"[VALIDATION] Image-only PDF detected: {file_path} (text length: {total_text_length})")
        else:
            print(f"[VALIDATION] Text-based PDF detected: {file_path} (text length: {total_text_length})")
        
        return is_image_only
        
    except Exception as e:
        print(f"[VALIDATION] Error checking PDF: {e}")
        # If we can't check, assume it's okay to avoid false rejections
        return False
...
def insert_employee():
    ...
    try:
        ...
        if file:
            ...
            # VALIDATION: Check file size (3MB limit)
            file_size = os.path.getsize(temp_original)
            max_size = 3 * 1024 * 1024  # 3 MB
            if file_size > max_size:
                os.unlink(temp_original)  # Clean up
                print(f"[insert_employee] ERROR: File too large: {file_size / 1024 / 1024:.2f} MB")
                return jsonify({
                    "ok": False, 
                    "error": "File too large. Please upload a CV under 3 MB."
                }), 400
            
            # VALIDATION: Check if PDF is image-only (Canva/scanned)
            if filename.lower().endswith('.pdf'):
                if is_image_only_pdf(temp_original):
                    ...
                    return jsonify({
                        "ok": False,
                        "error": "Image-only PDFs (like Canva or scanned CVs) are not allowed. Please upload a text-based CV."
                    }), 400
